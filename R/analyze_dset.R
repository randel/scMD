...
analyze <- function(method, q, n_markers,gamma, dmeths = NULL, verb = TRUE, normalize = TRUE,scale = scale,
  ...
  output <- lapply(datasets, function(dset) analyze_dset(dset, method =method, q = q,n_markers =n_markers, gamma=gamma,
                                                         dmeths = dmeths, verb = verb, normalize = normalize, customed_markers = customed_markers,markers_range = markers_range,batchcorrec = batchcorrec,scale = scale,rm.duplicated = rm.duplicated,mrkpen = mrkpen ))
  end_time <- Sys.time()
